...
namespace WLB_Builder.Models
{
    class OLE
    {
        ...
        /*public static void export(String pptFile, String destination)
        {
            Package pkg = Package.Open(pptFile);
            foreach (PackagePart pkgprt in pkg.GetParts())
            {
                if(pkgprt.Uri.ToString().StartsWith("/ppt/embeddings/"))
                {
                    ...
                    string targetFile = destination + pkgprt.Uri.ToString().Remove(0, "/ppt/embeddings/".Length);
        ...
        static void ProcessPackage(IStorage pStg, String destination)
        {
            ...
            do
            {
                ...
                if (numReturned != 0)
                {
                    ...
                    if (ss[0].pwcsName.Contains("Ole10Native") == true)
                    {
                        ...
                        // Create the file
                        System.IO.BinaryWriter bWriter = new System.IO.BinaryWriter(System.IO.File.Open(destination + path, System.IO.FileMode.Create));
